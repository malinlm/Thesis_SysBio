...
def compute_pe_truvada(indicator, tend, adh_pattern=None, time_step=0.02, ode_solver=euler):
    ...
    e_rai.add_regimen(r_ftc)
    e_rai.add_regimen(r_tdf)
    ...
    if time_step and ode_solver:
        e_rai.set_pk_ode_solver('TDF', ode_solver)
        e_rai.set_pk_time_step('TDF', time_step)
    e_rai.compute_concentration()
    #e_rai = deepcopy(e_rvi)

    if indicator[1]:    # set local PK ratio in colon tissue cells
    ...
    if indicator[2]:    # set drug effect file (MMOA) for different local dNTP level
        ...
        e_rai.set_pd_file('Data/comb_output_ColChen.csv')
...
def process_one_dose(dose, indicator,  dosing_file='dosing.csv'):
    """
